...
exports.registerValidator = zod_1.z.object({
    ...
    date_of_birth: zod_1.z
        .string()
        .regex(/^\d{4}\/(0[1-9]|1[0-2])\/(0[1-9]|[12]\d|3[01])$/, "Date of birth must be in YYYY/MM/DD format.")
        .refine((val) => {
    ...
    address: zod_1.z.string().trim().max(500, "Address cannot exceed 500 characters."),
    gender: zod_1.z.string().trim().max(20, "Gender cannot exceed 20 characters."),
    ...
    emergency_contact_number: zod_1.z
        .string()
        .regex(/^09\d{9}$/, "Emergency contact number must be a valid Philippine mobile number.")
        .optional(),
});
